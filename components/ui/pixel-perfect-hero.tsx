...
export function PixelHero({
  ...
  return (
    <div className="relative w-full min-h-[100dvh] hero-dark-radial flex flex-col justify-between md:justify-center md:gap-6 py-8 md:py-0 px-2 sm:px-6 overflow-hidden select-none isolate">
      {/* Pixel canvas — emerald green tones */}
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_20%,rgba(1,15,7,0.6)_100%)] pointer-events-none" />
      </div>

      {/* Emerald glow orbs */}
      <div className="pointer-events-none absolute -top-40 left-1/2 -translate-x-1/2 h-[500px] w-[700px] rounded-full bg-emerald-500/10 blur-[120px] z-0" />
      <div className="pointer-events-none absolute bottom-20 left-0 h-80 w-96 rounded-full bg-teal-600/10 blur-[100px] z-0" />
      <div className="pointer-events-none absolute bottom-20 right-0 h-80 w-96 rounded-full bg-green-700/10 blur-[100px] z-0" />

      {/* Badge + Heading */}
      <div className="flex flex-col items-center justify-center text-center order-1 md:order-1 mt-24 sm:mt-0 pointer-events-none w-full z-10">
        <div
          className={cn(
            "mb-6 inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-1.5 text-sm font-medium text-emerald-300 backdrop-blur-sm transition-all duration-700",
            isLoaded ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"
          )}
          style={{ transitionDelay: "100ms" }}
        >
          <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse" />
          Powered by Gemini AI · Built for a greener planet
        </div>

        <h1
          className={cn(
            "canopy-impact-text flex flex-row items-center justify-center gap-2 sm:gap-5 lg:gap-6 px-1 w-full flex-wrap",
            "text-[3rem] xs:text-[3.5rem] sm:text-6xl md:text-8xl lg:text-9xl leading-none",
      ...
      <div className="flex flex-col items-center justify-center text-center my-auto md:my-0 order-2 md:order-2 px-1 w-full pointer-events-none z-10">
        <p
          className={cn(
            "text-sm sm:text-lg md:text-xl font-light text-emerald-100/80 max-w-[95%] sm:max-w-md md:max-w-xl px-1 leading-relaxed",
            "transition-all duration-1000",
            isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
          )}
        ...
        <div
          className={cn(
            "mt-8 flex items-center gap-6 sm:gap-10 transition-all duration-1000",
            isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
          )}
          style={{ transitionDelay: "380ms" }}
        >
          {[
            { value: "2.4×", label: "avg. CO₂ saved" },
            { value: "10k+", label: "active trees" },
            { value: "94%", label: "user satisfaction" },
          ].map((s) => (
            <div key={s.label} className="text-center">
              <p className="text-xl sm:text-2xl font-bold text-emerald-300">{s.value}</p>
      ...
      <div
        className={cn(
          "pointer-events-auto flex flex-row items-center justify-center gap-3 mt-4 md:mt-10 mb-8 md:mb-0 order-4 md:order-3 transition-all duration-1000 transform px-1 z-10",
      ...
      >
        <button
          ...
          className="relative inline-flex h-11 items-center justify-center gap-2 rounded-xl px-5 md:px-9 text-sm font-semibold text-white cursor-pointer overflow-hidden group
            bg-gradient-to-br from-emerald-500 to-emerald-700
            shadow-[0_0_24px_rgba(16,185,129,0.5),0_4px_16px_rgba(0,0,0,0.3)]
            hover:shadow-[0_0_36px_rgba(16,185,129,0.7)]
            transition-all duration-300 hover:scale-[1.03] active:scale-[0.97]
            ring-1 ring-emerald-400/40"
        >
          <span className="absolute inset-0 bg-gradient-to-t from-white/0 to-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl" />
          <span className="inline md:hidden relative z-10">{primaryCtaMobile}</span>
          <span className="hidden md:inline relative z-10">{primaryCta}</span>
          <ArrowRight className="w-4 h-4 relative z-10 group-hover:translate-x-0.5 transition-transform duration-200" />
        </button>

        <button
          onClick={onSecondaryClick}
          className="relative inline-flex h-11 items-center justify-center gap-2 rounded-xl px-5 md:px-9 text-sm font-semibold cursor-pointer
            bg-white/8 backdrop-blur-md text-emerald-100
            border border-emerald-500/25
            hover:bg-white/14 hover:border-emerald-400/40
            transition-all duration-300 hover:scale-[1.03] active:scale-[0.97]"
        >
      ...
      <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[#f0fdf4] to-transparent pointer-events-none z-10" />
    </div>
  );
}
